  ...
  arr.map((value) => {
    res += `
      ...
      value.name.length > 20 ? value.name.slice(0, 20) + "..." : value.name
                    ...
                    <a href="./details.html" data-fullname="${value?.name}" class="px-3 py-2 w-full bg-cyan-600 rounded-xl text-[20px] font-medium mt-4 dark:text-[white]" id="country-card-btn"> Details </a>
                </div>
            </div>
        `;
  });
